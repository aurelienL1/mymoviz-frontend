...
function Home() {
  ...
  const [moviesData,setMoviesData] = useState([]);
    useEffect(()=> {
      fetch('https://mymoviz-backend-drab-iota.vercel.app/movies')
  ...
  const movies = moviesData.map((data, i) => {
    ...
    if (overview.length > 250){
      overview = overview.substring(1,250)+".."
    }
    
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.original_title} overview={overview} poster={poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
